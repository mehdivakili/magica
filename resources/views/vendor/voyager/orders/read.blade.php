...
@section('page_header')
    <h1 class="page-title">
        ...
        <a href="{{ route('download_order',[$dataTypeContent->getKey()]) }}" style="margin: 0" class="btn btn-primary">
            <span class="glyphicon glyphicon-download"></span>&nbsp;
            {{ __('download') }}
        </a>
        <form style="display: flex;flex-direction: row" action="{{route('upload_order',[$dataTypeContent->getKey()])}}"
              method="post" enctype="multipart/form-data">
            @csrf
            <input name="file" type="file" class="form-control form-control-file">
            <input type="submit" href="{{ route('upload_order',[$dataTypeContent->getKey()]) }}"
                   style="margin-left: 15px" class="btn btn-success glyphicon glyphicon-list"
                   value="{{ __('upload') }}">
            {{--            <span class="glyphicon glyphicon-list"></span>&nbsp;
                        {{ __('upload') }}
                    </input>--}}
...
@section('content')
    <div class="page-content read container-fluid" style="margin-top: 5px">
        <div class="row">
            <div class="col-md-12">
                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'relationship')
                                @include('vendor.voyager.FormFields.relationship', ['view' => 'read', 'options' => $row->details])
                            ...
                            @else
                                ...
                                @if(!isset($row->details->a))
                                    <p>{{ $dataTypeContent->{$row->field} }}</p>
                                @else
                                    <a href="{{route('admin_get_picture_with_link',[$dataTypeContent->{$row->field}]) }}">{{ $dataTypeContent->{$row->field} }}</a>
                                @endif
                            @endif
                        </div><!-- panel-body -->
                        @if(!$loop->last)
