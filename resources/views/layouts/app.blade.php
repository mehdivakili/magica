...
<head>
    ...
    <link rel="icon" href="{{url('storage/magica_m.svg')}}">
    <!-- Scripts -->
...
@empty($js)
    @else
        @foreach($js as $j)
            <script src="{{get_js_link($j)}}" defer></script>
        @endforeach
    @endempty
...
<!-- Styles -->
    ...
    <link href="{{ get_css_link('app') }}" rel="stylesheet">
...
<div id="panel-app" class="panel">
    ...
    <div class="panel-menu-holder"></div>
    <div class="panel-menu style-div" style="">
        ...
        <div class="style-div" style="margin: -20px; padding: 0; height: 150%;">
            ...
            {{menu('panel_menu','panel_menu',['mobile'=>false])}}
...
{{--            <div class="magica-con"><a href="{{route('home')}}">@include('svg.magica_delta2')</a>
            ...
            {{menu('panel_menu','panel_menu',['mobile'=>true])}}
...
<script src="{{ get_js_link('app') }}"></script>
...
<script>
    ...
    function bookmarkGray(url, e) {
        $.ajax({
            type: 'GET',
            url: url,
            success: function (data) {
                if (data === "added successfully") {
                    $(e).html(`@include("svg.bookmark-fill-gray")`);
                } else {
                    $(e).html(`@include("svg.bookmark-gray")`);
                }
